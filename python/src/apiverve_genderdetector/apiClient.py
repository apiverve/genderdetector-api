...
class GenderdetectorAPIClientError(Exception):
...
class ValidationError(GenderdetectorAPIClientError):
...
class GenderdetectorAPIClient:
    ...
    VALIDATION_RULES = {"name": {"type": "string", "required": True}, "country": {"type": "string", "required": False, "minLength": 2, "maxLength": 2, "default": "us"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the GenderdetectorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises GenderdetectorAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise GenderdetectorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[GenderdetectorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns GenderDetectorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises GenderdetectorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise GenderdetectorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise GenderdetectorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise GenderdetectorAPIClientError(f"Request failed: {str(e)}")
